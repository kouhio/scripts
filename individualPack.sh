...
printBaseData() {
    ...
    printToFile "    printf \"%0\${PLEN}d/%0\${PLEN}d \$(date +%T): Removing %s\\n\" \"\${COUNTED_ITEMS}\" \"\${MAX_ITEMS}\" \"\$1\""
    printToFile "    REMFSAVE=i\$(du -k \"\$1\" | cut -f1)"
    printToFile "    GLOBAL_FILESAVE=\$((GLOBAL_FILESAVE + REMFSAVE))"
    printToFile "    rm \"\$1\""
    printToFile "  elif [ \"\$INPUTFILE\" == \"mv\" ]; then"
    printToFile "    [ ! -f \"\$1\" ] && return 0"
    printToFile "    CLEARNAME=\$(echo \"\$1\" | tr -dc '[:alnum:]\n\r' | tr '[:upper:]' '[:lower:]')"
    printToFile "    printf \"%0\${PLEN}d/%0\${PLEN}d \$(date +%T): Renaming '%s' to '\$CLEARNAME'\\n\" \"\${COUNTED_ITEMS}\" \"\${MAX_ITEMS}\" \"\$1\""
